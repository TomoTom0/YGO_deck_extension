...
def updateCardInfosAll_Langs(cardInfos_Langs_old={},term_dics_Langs={} ,langs=["ja", "en", "de", "fr", "it", "es", "pt", "ko"], paraIsValid=True):
...
def obtainCardInfosAll_fromYGODB(page, lang="ja", rp=2000, term_dic={}):
    ...
    for _count_tmp in range(2):
        ...
        if list(enc_cid_dic.values())[0] is not None:
            break
        else:
            _, enc_cid_dic=_obtainSoup()
    print(list(enc_cid_dic.values())[0])
...
def make_good_db2(on_regular=True, addInfo={"ocg": {}, "uploadIsValid":True}, githubReuse={"db":True, "Lang":True}, modifyOldIsValid=True, 
    ...
    if on_regular and (lastUpdated == f"{datetime.date.today()}" or datetime.date.today().weekday() % 3 != 2):
        ...
        encImgs=cardInfos_Langs["ja"]["encImg"]
        if any(s is None for s in encImgs):
            print("some encImgs are invalid.")
            return None
...
if __name__ == "__main__":
    data_new = make_good_db2(True, githubReuse={"db":True, "Lang":True}, modifyOldIsValid=True, paraIsValid=False)
